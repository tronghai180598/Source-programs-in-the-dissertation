...
void parse(String str){
  ...
  if (str.substring(0,3) == "as:"){
    ...
    if (str.substring(3,6) == nms[e_Kpv]) {  ctrl.Kpv = 0.001 * str.substring(7).toInt(); return; }
    if (str.substring(3,6) == nms[e_Kdv]) {  ctrl.Kdv = 0.001 * str.substring(7).toInt(); return; }
  ...
  if (str.substring(0,3) == "ag:"){
    ...
    if (str.substring(3,6) == nms[e_Kpv]) rslt = int(ctrl.Kpv * 1000);
    if (str.substring(3,6) == nms[e_Kdv]) rslt = int(ctrl.Kdv * 1000);
...
void doParser(){
  ...
  if(gLog == 3){ 
    Serial.print(millis()); Serial.print("\t"); 
    Serial.print(ctrl.lTr); Serial.print("\t"); 
    Serial.print(gx); Serial.print("\t");
    Serial.println(ctrl.mVi);Serial.print("\t"); 

  }
  if(gLog == 4){ 
    Serial.print(millis()); Serial.print("\t"); 
    Serial.print(ay); Serial.print("\t"); 
    Serial.println(ctrl.mFi);  
  }
  if(gLog == 5){ 
    Serial.print(millis());Serial.print("\t"); 
    Serial.print(ctrl.uM); Serial.print("\t"); 
    Serial.print(ctrl.Ui); Serial.print("\t");
    Serial.println(ctrl.uI);
...
void loop() {
  ...
  if(dt > 0){
   ...
   // ComplementaryFilter(&Acc_filter, 1.0 * gx, 1.0 * ay, fDt);
    ctrl.UdateKalman(1.0 * ay, 1.0 * gx);
    float uM = ctrl.updateCtrl(fDt, setFi, ctrl.mFi, ctrl.mVi);
   // mdl.updateMdl(fDt, ctrl.Umi);
  }
