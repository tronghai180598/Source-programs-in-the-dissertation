...
void integr(float dt, float inp, float tm, float& out){  out += inp * dt / tm; };

void intert(float dt, float inp, float k, float tm, float& out){  out += (inp * k - out) * dt / tm; };

void rldiff(float dt, float dinp, float tdf, float tm, float& out){ out += dinp * tdf/tm - out * dt / tm; };
...
KrenMdl::KrenMdl() {  mTf = 0.2; mTv = 0.015; mTm = 0.03; mFi = 0; mVi = 0; mAc = 0;};
...
float KrenMdl::updateMdl(float dt, float inp, float ul) {
  ...
  integr(dt, mAc + 
    //((nFi==0)?0:(2000*sin(mTms*425) +  3*ul*sin(mTms*125))), mTv, mVi);
    //((nFi==0)?0:(2000*sin(mTms*425) +  0.3*ul*sin(mTms*125))), mTv, mVi);
    //((nFi==0)?0:(200*sin(mTms*425) +  0.3*ul*sin(mTms*125))), mTv, mVi);
    ((nFi==0)?0:(20*sin(mTms*425) +  0.03*ul*sin(mTms*125))), mTv, mVi);
    //0, mTv, mVi);
...
char* KrenMdl::print() {
  sprintf(msg, " %f %f %f", getFi()/100, getVi()/100, mAc * 32 / 100);
  return msg;
};
